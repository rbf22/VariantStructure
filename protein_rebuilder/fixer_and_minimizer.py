...
class FixerMinimizer:
    ...
    def _create_system(self, topology):
        ...
        if topology.getPeriodicBoxVectors():
            system = forcefield.createSystem(
                ...
                nonbondedCutoff=1.0 * unit.nanometer,  # pylint: disable=no-member
    ...
    def _add_restraints(self, system, topology, positions):
        """Add C-alpha restraints to the system."""
    ...
    def _run_simulation(self, topology, system, positions, niter_minimize):
    ...
    def fix_and_minimize(  # pylint: disable=too-many-locals
        self, biopy_struct, keep_water=True, niter_minimize=500, restrain_calpha=True
    ):
        """Fix and minimize a protein structure."""
        ...
        if restrain_calpha:
            self._add_restraints(system, topology, positions)

        minimized_positions = self._run_simulation(
            topology, system, positions, niter_minimize
        )

        with tempfile.NamedTemporaryFile(
            delete=False, suffix=".pdb", mode="w", encoding="utf-8"
        ) as out_pdb_path:
            app.PDBFile.writeFile(topology, minimized_positions, out_pdb_path)
