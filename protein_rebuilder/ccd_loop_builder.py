...
The approach:
- Build an initial chain of N/CA/C/O atoms for each inserted residue
  using ideal bond lengths/angles.
- Use CCD altering phi dihedrals of inserted residues to move the chain
  tip to target.
...
def dihedral(p0, p1, p2, p3):
    """Calculate the dihedral angle in degrees between 4 points."""
    b0 = -1.0 * (p1 - p0)
    b1 = p2 - p1
    b2 = p3 - p2
    # normalize b1
...
def set_dihedral(p0, p1, p2, p3, new_angle_deg):
    """Rotate p3 around the p1-p2 axis to set a new dihedral angle."""
...
def build_peptide_fragment(
    n_residues, start_ca=np.array([0.0, 0.0, 0.0]), direction=np.array([1.0, 0.0, 0.0])
):
    """
    Create an initial straight fragment of n_residues backbone coordinates.

    Uses ideal geometry. Returns list of residues where each residue is
    a dict of atom name -> 3-vector.
    """
    residues = []
...
def ccd_close_loop(  # pylint: disable=too-many-locals,inconsistent-return-statements
    insert_coords, anchor_next_coords, max_iter=200, tol=1e-3
):
    """
    ...
    We attempt to adjust phi dihedrals of the inserted residues via CCD
    to bring the last built N atom close to the next anchor N atom.
